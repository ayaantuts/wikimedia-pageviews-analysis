...
class EventDetectionModel:
	...
	def generate_pseudo_labels(self, df, z_threshold=2, min_editors=0):
		"""
	...
	def train_with_smote(self, df):
		"""
		Fixes the '0 Precision / 0 Recall' problem using SMOTE and Threshold Tuning.
		"""
		df_labeled = self.generate_pseudo_labels(df)
		X = df_labeled[self.feature_cols].fillna(0)
		y = df_labeled['is_event']
		
		# 1. Split Time-Series (Train on old, Test on new)
		split_idx = int(len(df) * 0.8)
		X_train, X_test = X.iloc[:split_idx], X.iloc[split_idx:]
		y_train, y_test = y.iloc[:split_idx], y.iloc[split_idx:]
		
		# 2. Apply SMOTE (ONLY on Training Data!)
		# This creates fake "Event" examples so the model sees them 50/50
		smote = SMOTE(random_state=42, sampling_strategy=0.5) # Make events 50% of data
		...
		# 4. Threshold Tuning (The Magic Fix)
		# Instead of using default predict() (threshold=0.5), we find the best one.
		...
		optimal_idx = np.argmax(recalls < 0.50) # simple heuristic, tunable
		best_threshold = thresholds[optimal_idx]
		
		print(f"✅ Optimal Decision Threshold found: {best_threshold:.4f}")
